...
def plot_tangle(collision_number, numerical_tangle_list, tangle_lb_list, fidelity_GHZ_list, fidelity_T_list,
                 numerical_tangle_list_sim, fidelity_GHZ_list_sim, fidelity_T_list_sim,
                 theta, initial_statevector, fig, i, labels, j, min_GHZ_fidelity, min_T_fidelity):
    ...
    if j == 'tangle':
        ...
        ax.plot(collision_number, numerical_tangle_list, ls='', ms=s1, mew=mew1, marker='x', label=r'ibmq\_16\_melbourne', clip_on=False, color=c1, zorder=5)
    ...
    if j == 'GHZ_fidelity':
        ...
        ax.plot(collision_number, fidelity_GHZ_list, ls='', ms=s1, mew=mew1, marker='x', label=r'ibmq\_16\_melbourne',
    ...
    if j == 'fidelity-tangle':
        ...
        ax.set_ylabel(r'Fidelity ($F$)')
        ax.plot(numerical_tangle_list, fidelity_GHZ_list, ls='', ms=s1, mew=mew1, marker='x', label=r'ibmq\_16\_melbourne',
                   clip_on=False, color=c1, zorder=5)
...
f_t = []
for i in range(1, 5):
    f_g.append(pickle.load(open("plots/data{}.json".format(i), "rb"))['fidelity_GHZ_list'])
    f_t.append(pickle.load(open("plots/data{}.json".format(i), "rb"))['fidelity_T_list'])
min_GHZ_fidelity = np.min(f_g)
...
for j in plots:
    ...
    for i in range(1, 5):
        ...
        plot_tangle(data['collision_number'], data['numerical_tangle_list'], None, data['fidelity_GHZ_list'],
                    ...
                    min_GHZ_fidelity, min_T_fidelity)
