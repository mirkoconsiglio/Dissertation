...
def plot_results(collision_number, numerical_tangle_list, tangle_lb_list, fidelity_GHZ_list, fidelity_T_list,
    ...
    ax.plot(collision_number, fidelity_GHZ_list, ls='', ms=s1, marker='x', mew=mew1, label=r'ibmq\_16\_melbourne', clip_on=False,
               color=c1, zorder=5)
    ax.plot(collision_number, fidelity_GHZ_list_sim, ls='', ms=s2, marker='+', mew=mew2, label=r'qasm\_simulator', clip_on=False,
               color=c2, zorder=4)
    ax.plot(x, F_GHZ, label=r'Theoretical', color=c3, ls='-', lw=lw, zorder=3)
    p, pcov = curve_fit(h, collision_number, fidelity_GHZ_list, bounds=[[1, 0, 2], [np.inf, np.inf, np.inf]])
    y = h(x, p[0], p[1], p[2])
    #ax.plot(x, y, label=r'Fitted Curve', color=c4, ls='--', lw=lw, zorder=3)
    ax.legend()
    ...
    ax.set_ylabel(r'Fidelity ($F$)')
    ax.plot(numerical_tangle_list, fidelity_GHZ_list, ls='', ms=s1, marker='x', mew=mew1, label=r'ibmq\_16\_melbourne', clip_on=False,
               color=c1, zorder=5)
    ax.plot(numerical_tangle_list_sim, fidelity_GHZ_list_sim, ls='', ms=s2, marker='+', mew=mew2, label=r'qasm\_simulator', clip_on=False,
               color=c2, zorder=4)
