...
def plot_results(collision_number, numerical_tangle_list, tangle_lb_list, fidelity_GHZ_list, fidelity_T_list,
                 ...
                 theta, initial_statevector, rho_GHZ_list):
    ...
    ax.plot(collision_number, numerical_tangle_list, ls='', ms=s1, marker='+', mew=mew1, label=r'ibmq\_16\_melbourne', clip_on=False,
               c=c1, zorder=5)
    ax.plot(collision_number, numerical_tangle_list_sim, ls='', ms=s2, marker='x', mew=mew2, label=r'qasm\_simulator', clip_on=False,
               c=c2, zorder=4)
    ...
    ax.plot(collision_number, fidelity_GHZ_list, ls='', ms=s1, marker='+', mew=mew1, label=r'ibmq\_16\_melbourne', clip_on=False,
               color=c1, zorder=5)
    ax.plot(collision_number, fidelity_GHZ_list_sim, ls='', ms=s2, marker='x', mew=mew2, label=r'qasm\_simulator', clip_on=False,
               color=c2, zorder=4)
    ax.plot(x, F_GHZ, label=r'Theoretical', color=c3, ls='-', lw=lw, zorder=3)
    p = curve_fit(h, collision_number, fidelity_GHZ_list, bounds=[[2, 0], [np.inf, np.inf]])[0]
    print(p)
    y = h(x, p[0], p[1])
    ...
    ax.yaxis.set_ticks(np.linspace(a, 1, int((1 - a) * 10 + 1)))
    ax.set_ylabel(r'Fidelity ($F$)')
    ax.plot(collision_number, fidelity_T_list, ls='', ms=s1, marker='+', mew=mew1, label=r'ibmq\_16\_melbourne', clip_on=False,
               color=c1, zorder=5)
    ...
    ax.set_ylabel(r'Fidelity ($F$)')
    ax.plot(numerical_tangle_list, fidelity_GHZ_list, ls='', ms=s1, marker='+', mew=mew1, label=r'ibmq\_16\_melbourne', clip_on=False,
               color=c1, zorder=5)
    ax.plot(numerical_tangle_list_sim, fidelity_GHZ_list_sim, ls='', ms=s2, marker='x', mew=mew2, label=r'qasm\_simulator', clip_on=False,
               color=c2, zorder=4)
...
plot_results(data['collision_number'], data['numerical_tangle_list'], None, data['fidelity_GHZ_list'],
             ...
             data['fidelity_T_list_sim'], data['theta'], data['initial_statevector'], data['rho_GHZ_list'])